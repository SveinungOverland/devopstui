...
// drill-down view.
var footerItemDesc = []key.Binding{keys.Focus, keys.Desc, keys.Comments, keys.New, keys.NewBug, keys.Edit, keys.Title, keys.State, keys.Assign, keys.Preview, keys.PreviewUp, keys.PreviewDown, keys.Back, keys.Help}
var footerItemKanban = []key.Binding{keys.Left, keys.Right, keys.Up, keys.Down, keys.ColLeft, keys.ColRight, keys.New, keys.NewBug, keys.Details, keys.Edit, keys.State, keys.Focus, keys.Comments, keys.Back}
